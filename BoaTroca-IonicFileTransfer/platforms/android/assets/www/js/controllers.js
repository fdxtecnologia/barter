...
.controller('menuCtrl', function($scope, $rootScope, $http, $state, $window, OpenFB) {
    ...
    $rootScope.$on('$stateChangeStart', function(event, toState) {
        if (toState.name !== "app.login" && toState.name !== "app.logout" && !$window.sessionStorage['fbtoken']) {
            ...
            $http({
                method: 'GET',
                url: 'http://ec2-54-187-242-58.us-west-2.compute.amazonaws.com:8080/barterserver/trade/post/list',
                params: userJson
            })
        ...
        OpenFB.login('email,user_birthday').then(
            function(success) {
                OpenFB.get('/me').success(function(user) {
                    ...
                    $http({
                        method: 'GET',
                        url: 'http://ec2-54-187-242-58.us-west-2.compute.amazonaws.com:8080/barterserver/user/post/save',
                        params: userJson
                    }).
                    success(function(data, status, headers, config) {
...
.controller('ShowPictureCtrl', function($scope, $state, $stateParams, $http, $rootScope, $ionicLoading, $location) {
    ...
    $scope.removeItem = function() {
        ...
        $http({
            method: 'GET',
            url: 'http://ec2-54-187-242-58.us-west-2.compute.amazonaws.com:8080/barterserver/user/picture/delete',
            params: picJson
        })
            .success(function(data, status, headers, config) {
                $scope.list = [];
...
.controller('ChatCtrl', function($scope, $rootScope, $http, $ionicLoading, $interval, $ionicScrollDelegate, $ionicSideMenuDelegate) {
    ...
    $scope.sendMessage = function() {
        if ($scope.dados.message != "") {
            ...
            $http({
                method: 'POST',
                url: 'http://ec2-54-187-242-58.us-west-2.compute.amazonaws.com:8080/barterserver/user/new/message',
                params: msgJson
            })
                .success(function() {
                    loadChat();
    ...
    function loadChat() {
        ...
        $http({
            method: 'GET',
            url: 'http://ec2-54-187-242-58.us-west-2.compute.amazonaws.com:8080/barterserver/user/chat',
            params: chatJson
        })
            .success(function(data, status, headers, config) {
...
.controller('RegisterPicturerCtrl', function($scope, $rootScope, $state, $http, $ionicLoading, $location) {
    ...
    $scope.onUploadSucess = function() {
        var picJson = {
            ...
            'picture.photoURL': "http://ec2-54-187-242-58.us-west-2.compute.amazonaws.com/pictures/" + $scope.ActualPicId + ".jpg"
        };
        $http({
            method: 'GET',
